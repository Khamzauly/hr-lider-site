...
test('buildGoogleAnalyticsScripts returns null when every id is missing', () => {
  assert.equal(buildGoogleAnalyticsScripts('', ''), null);
  assert.equal(buildGoogleAnalyticsScripts(undefined, undefined), null);
});
...
test('buildGoogleAnalyticsScripts can configure Google Ads tag alongside GA4', () => {
  const scripts = buildGoogleAnalyticsScripts('G-TEST123', 'AW-123456789');

  assert.equal(scripts.src, 'https://www.googletagmanager.com/gtag/js?id=G-TEST123');
  assert.match(scripts.inline, /gtag\('config', 'G-TEST123'\);/);
  assert.match(scripts.inline, /gtag\('config', 'AW-123456789'\);/);
});

test('buildGoogleAnalyticsScripts can load Google Ads tag when GA4 is absent', () => {
  const scripts = buildGoogleAnalyticsScripts('', 'AW-123456789');

  assert.equal(scripts.src, 'https://www.googletagmanager.com/gtag/js?id=AW-123456789');
  assert.match(scripts.inline, /gtag\('config', 'AW-123456789'\);/);
});
